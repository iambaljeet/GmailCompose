...
val drawerUiState = DrawerUiState(
        drawerOptions = drawerOptionsList,
)

val homeUiState = HomeUiState(
        allMails = mailList.plus(mailList)
)

fun mailDetailsUiState(mailId: Long?): MailDetailsUiState {
    val mailData = mailList.find { it.mailId == mailId }
    return MailDetailsUiState(
            mailData = mailData!!
    )
}